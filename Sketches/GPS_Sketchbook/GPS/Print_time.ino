...
int print_time(){
    ...
    hour1.numb(int(hr/10) % 10);
    //Serial.print(int(hr/10) % 10);
    //Serial.print(int(hr % 10),0);
    hour0.numb(int(round(hr % 10)));
    //Serial.print(':');
    min1.numb(int(round((now.minute()/10) % 10)));
    //Serial.print(round((now.minute()/10) % 10),0);
    min0.numb(int(round(now.minute() % 10)));
    //Serial.print(int(round(now.minute() % 10)));
    //Serial.print(':');
    sec1.numb(int(round((now.second()/10) % 10)));
    //Serial.print(int(round((now.second()/10) % 10)));
    sec0.numb(int(round(now.second() % 10)));
